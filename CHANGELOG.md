...
### Added
- Cross-platform sandbox execution boundary (Linux, macOS, Windows) under unified typestate pipeline: `UnpreparedProductionExecution` -> `prepare` -> `PreparedProductionExecution::spawn` -> `wait_collect/finish`.
- **macOS Backend**: Seatbelt profile sandbox (`sandbox-exec`), process-group isolation, group-death sweep, network lockdown (`--net off`).
- **Windows Backend**: Windows Job Objects tree containment with kill-on-close, AppContainer process boundary (`--net off`), stdin-safe process supervision.
- Hardened `mcp wrap` on Windows against ENV-POISON attacks with canonical fallback to `C:\Windows` and `C:\Program Files` and `System32` binary validation.
- Real remediation diagnostics in `vetto doctor --fix` for macOS (Seatbelt / SIP / macOS 12+) and Windows (Job Objects / Hyper-V / BIOS virtualization).

### Security Matrix & Platform Capabilities
- **Linux (Tier 1)**: Landlock VFS isolation, seccomp `UnixOnly` / `AgentMin`, rlimit ceilings (AS, NPROC, CPU, FSIZE), sticky `NO_NEW_PRIVS`, process groups, full `/proc` nonce tree sweep. (PROVEN)
- **macOS (Tier 2)**: Seatbelt write isolation, network lockdown (`--net off`), pgroup host verification, group-death tree cleanup. Unprivileged read-masking and syscall filtering remain Unsupported (platform limitation).
- **Windows (Tier 3)**: Job Objects tree containment, AppContainer filesystem/process/net-off boundary, policy-conditional ceilings. Syscall filtering and exec-root isolation remain Unsupported.

### Fixed
- Eliminated PID ownership race in Linux `scan_nonce_pids` where concurrent sweeps could reap unconfirmed foreign zombie processes.
- Resolved 40-minute stdin hang in Windows CI caused by headless `timeout` and premature blocking `child.wait()`.
- Removed duplicate unreachable `seatbelt` return in `production.rs` (macOS clippy).
- Configured explicit Gitleaks allowlists for mock/synthetic secret scanner test fixtures.
