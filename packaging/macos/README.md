...
3. **Building the `.pkg` installer** using `pkgbuild`:
   ```bash
   ./packaging/macos/build_pkg.sh 0.4.3 aarch64-apple-darwin
   ```
4. **Submitting for Notarization** via `xcrun notarytool`:
   ```bash
   xcrun notarytool submit target/pkg_out/vetto-0.4.3-aarch64-apple-darwin.pkg \
...
5. **Stapling the Ticket**:
   ```bash
   xcrun stapler staple target/pkg_out/vetto-0.4.3-aarch64-apple-darwin.pkg
   ```
6. **Verifying with Gatekeeper**:
   ```bash
   spctl --assess --type install --verbose target/pkg_out/vetto-0.4.3-aarch64-apple-darwin.pkg
   ```
