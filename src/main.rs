...
fn doctor(probe_deny: bool, check_agent: Option<&str>, fix: bool) -> Result<()> {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // Uniform backend probe: WSL2 distro + guest vetto (Tier-1 inside
        // the guest, fail-closed with an action when unavailable).
        {
            let avail = sandbox::wsl2::Wsl2Sandbox::probe_availability();
            println!(
                "wsl2 tier-1 backend:     {}",
                if avail.available {
                    "available".to_string()
                } else {
                    format!("unavailable ({})", avail.reason)
                }
            );
            if !avail.available {
                println!("  action:                install WSL2, import the distro, install the Linux vetto inside; explicit `--backend process` keeps the deprecated AppContainer path");
            }
        }
        if fix {
            vetto::doctor::print_fixes(&[]);
