...
fn test_macos_pkg_script_exists_and_content() {
    ...
    assert!(
        content.contains("productbuild"),
        "Missing productbuild command"
    );
    assert!(
        content.contains("xcrun notarytool submit"),
        "Missing notarytool submit"
    );
    assert!(
        content.contains("--wait"),
        "Missing --wait flag in notarytool"
    );
    assert!(
        content.contains("xcrun stapler staple"),
        "Missing stapler command"
    );
}
...
fn test_windows_signing_script_exists_and_content() {
    ...
    let content = fs::read_to_string(script_path).unwrap();
    assert!(
        content.contains("signtool.exe"),
        "Missing signtool.exe command"
    );
    assert!(content.contains("/tr"), "Missing timestamp server flag");
    assert!(
        content.contains("RFC 3161") || content.contains("timestamp.digicert.com"),
        "Missing RFC 3161 or Digicert timestamp server"
    );
    assert!(
        content.contains("CertificateThumbprint"),
        "Missing CertificateThumbprint parameter"
    );
